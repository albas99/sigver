...
class IterableDataset(ABC):
    ...
    def iter_genuine(self, path: str) -> Iterable[Tuple[np.ndarray, str]]:
        """ Iterates through the genuine signatures

        Parameters
        ----------
        path : str
            The path where the signatures are stored
    ...
    @abstractmethod
    def iter_forgery(self, path: str) -> Iterable[Tuple[np.ndarray, str]]:
        """ Iterates through the skilled forgeries
